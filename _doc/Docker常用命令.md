...
docker start                              将一个已经终止的容器启动运行，后面跟ID或者NAME
docker stop                               终止一个运行中的容器
docker restart                           运行状态的容器重新启动
...
detach 在容器中退出，使用ctrl+p+q,使用exit会导致stop容器 
